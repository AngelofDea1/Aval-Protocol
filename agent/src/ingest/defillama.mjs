...
// Manual smoke check: `node agent/src/ingest/defillama.mjs uniswap`
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const slug = process.argv[2] ?? "uniswap";
  try {
    const series = await fetchFeeSeries(slug);
    const v = series.values;
    console.log(`slug              ${series.slug}`);
    console.log(`source            ${series.source}`);
    console.log(`datapoints        ${v.length}`);
    console.log(`first / last ts   ${series.points[0].timestamp} / ${series.points.at(-1).timestamp}`);
    console.log(`last 7 values     ${v.slice(-7).map((x) => x.toFixed(0)).join(", ")}`);
    console.log(`mean last 30d     ${(v.slice(-30).reduce((s, x) => s + x, 0) / Math.min(30, v.length)).toFixed(0)}`);
    console.log(`age (days)        ${obligorAgeDays(series).toFixed(0)}`);
    console.log(`temporal conc.    ${temporalConcentration(v).toFixed(4)}`);
  } catch (err) {
